...
class TransformerDecoderState(DecoderState):
    ...
    def load_state(self, state):
        for l, lc in state.items():
            for lk, lv in lc.items():
                self.cache[l][lk]=torch.tensor(lv,device=self.src.device)

    def unload_beam(self, beam_id):
        # It is expected that the size of first dimension of memory tensors is equal to beam size
        if not self.cache:
            return None
        res={}
        for l, lc in self.cache.items():
            res[l]={}
            for lk,lv in lc.items():
                res[l][lk]=lv[beam_id:beam_id + 1].tolist()
        return res
